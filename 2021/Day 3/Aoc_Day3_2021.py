# -*- coding: utf-8 -*-
"""
Created on Fri Dec  3 09:07:28 2021
...
def import_data():
    ...
    for line in f:
        diagnostic_data.append(line.strip())
    
    return diagnostic_data

data = import_data()

def calc_power(diagnostic_data):
    bit_len = len(diagnostic_data[0])
    gama_rate = {}
    epsilon_rate = {}
    power = 0
    ...
    for L in range(bit_len):
        ...
        zeros = 0
        ones = 0
        for K in diagnostic_data:
            temp_list.append(K[L])
        zeros = temp_list.count('0')
        ones = temp_list.count('1')
        if zeros > ones:
            gama_rate[L] = 0
            epsilon_rate[L] = 1
        else:
    ...
    gama_dec = conv_to_dec(gama_rate.values())
    epsi_dec = conv_to_dec(epsilon_rate.values())
    power = gama_dec * epsi_dec
    print(f'Sub power: {power}')
    return gama_rate,epsilon_rate

def conv_to_dec(bin_list):
    bin_string = ''
    for num in bin_list:
        bin_string += str(num)
