# -*- coding: utf-8 -*-
"""
Created on Tue Dec  7 13:25:11 2021
...
The crab submarines all need to be aligned before they'll have enough power to blast a large enough hole for your submarine to get through. However,
it doesn't look like they'll be aligned before the whale catches you! Maybe you can help?
...
def import_data():
    ...
    for line in f:
        crab_pos.append(line.split(","))
    
    for i in range(0, len(crab_pos[0])):
        crab_pos[0][i] = int(crab_pos[0][i])
    
    return crab_pos[0]

def optimize_pos(data_list):
    result_dict = {}
    ave_pos = round(sum(data_list)/len(data_list),0)
    print(data_list)
    print(f'average pos: {ave_pos}')
    while ave_pos != -1:
        temp = 0
        for crab in data_list:
            fuel_usage = abs(ave_pos - crab)
            temp += fuel_usage
        result_dict[ave_pos] = temp
        ave_pos -= 1
    
    return result_dict

#result = optimize_pos(import_data())
#print(result)
...
"""
def optimize_pos_2(data_list):
    result_dict = {}
    ave_pos = round(sum(data_list)/len(data_list),0)
    max_pos = max(data_list)
    list_range = []
    ...
    while ave_pos != -1:
        temp = 0
        for crab in data_list:
            sum_stop = int(abs(ave_pos - crab))
            fuel_usage = sum(list_range[0:sum_stop+1])
            temp += fuel_usage
        result_dict[ave_pos] = temp
        ave_pos -= 1
...
print(f'result for test data: {optimize_pos_2(test_list)}')
print(f'result for input data: {optimize_pos_2(import_data())}')