# -*- coding: utf-8 -*-
"""
Created on Wed Dec  1 14:07:50 2021
...
def find_measurements(depth_list):
    result = -1
    try:
        for L in range(len(depth_list)+1):
            if depth_list[L] < (depth_list[L+1]):
                result +=1
    except IndexError:
        last_index = len(depth_list) -1
        if depth_list[last_index] > depth_list[last_index-1]:
            result+=1
            return result
        else:
            return result
...
def find_sum(depth_list):
    sum_list = []
    #Populate Sum list for comparison
    try:
        for L in range(len(depth_list)):
            sum_list.append(depth_list[L] + depth_list[L+1] + depth_list[L+2])
    except IndexError:
        print(f'length of sum list: {len(sum_list)}')
        return sum_list
    
if __name__ == '__main__':
    print(find_measurements(depth_list))
    print(find_measurements(find_sum(depth_list)))