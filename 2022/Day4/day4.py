...
def loadData(filePath):
    ...
    for line in f:
        sectionAssignment.append(line.strip())

    return sectionAssignment

fullData = loadData(filePath)
testData = loadData(unitTestData)

def calcOverlap(cleaningdata):
    result = 0
    for item in cleaningdata:
        compareList = []
        tempList1 = []
        tempList2 = []
        tempnum = ""
        for L in item:
            if L =='-' or L == ',' or L == '':                    
                compareList.append(int(tempnum))
                tempnum = ""
            else:
                tempnum += L
        compareList.append(int(tempnum))
        ...
        for number in range(compareList[0],compareList[1]+1):
            tempList1.append(number)               
        for number in range(compareList[2], compareList[3]+1):
            tempList2.append(number)
        
        set1 = set(tempList1)
        set2 = set(tempList2)

        if set1.issubset(set2) == True or set2.issubset(set1) == True:
            result +=1

    return result

print(f"Test Passed = {calcOverlap(testData)==2}")
print(f"Full result = {calcOverlap(fullData)}")
