...
def loadData(filePath):
    ...
    for line in f:
        calorieList.append(line.strip())

    return calorieList

def calcCalories(calorieList):
    resultList = []
    result = 0

    for L,calorie in enumerate(calorieList):
        if calorie != "":
            result += int(calorie)
        else:
    ...
    print(resultList[0:5])
    return sum(resultList[0:3])

print(calcCalories(loadData(filePath)))
#print(calcCalories(loadData("unitTestData.txt")))
