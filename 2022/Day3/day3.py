...
ruckSacks = loadData(filePath)
#ruckSacks = loadData("unitTest.txt")

def calcRucksack(ruckSacks):
    priorityDict = {}
    charMatch = []
    prioritySum = 0
    ...
    for L, char in enumerate(alphaLower):
        priorityDict[char] = L + 1
    for L, char in enumerate(alphaUpper):
        priorityDict[char] = L + 27

    for items in ruckSacks:
        sackSize = len(items)
        sackHalf = int(sackSize/2)
        for char in items[0:sackHalf]:
            if char in items[sackHalf:]:
                charMatch.append(char)
                break
    for char in charMatch:
        prioritySum += priorityDict[char]

    return prioritySum

print(calcRucksack(ruckSacks))
...
"""
#To Do - Fix part 2
def calcRucksackGroups(ruckSacks):
    priorityDict = {}
    charMatch = []
    prioritySum = 0
    breakCount = 0
    ...
    for L, items in enumerate(ruckSacks):
        for char in items:
            try:
                if L%3 != 0:
                    breakCount += 1
                    break 
                elif (char in ruckSacks[L]) and (char in ruckSacks[L+1]) and (ruckSacks[L+2]):
                    matchCount += 1
                    charMatch.append(char)
                    break
            except IndexError:
                break
    print(f"Break Count = {breakCount} and Match count = {matchCount}")
    for char in charMatch:
        prioritySum += priorityDict[char]

    return prioritySum

print(calcRucksackGroups(ruckSacks))

def unitTest(testData):
    print(testData)
    if testData == 70:
...
unitTestData = calcRucksackGroups(loadData("unitTest.txt"))
